...
theorem primes_infinite : ∀ n, ∃ p > n, Nat.Prime p := by
  ...
  have : p ∣ 1 := by
    ...
    simp
  show False
  apply Nat.le_of_dvd zero_lt_one at this
  apply Nat.Prime.two_le at pp
  linarith
open Finset
...
theorem _root_.Nat.Prime.eq_of_dvd_of_prime {p q : ℕ}
  ...
  . apply Nat.Prime.eq_one_or_self_of_dvd prime_q p
    tauto
  . exact Nat.Prime.ne_one prime_p
...
theorem mem_of_dvd_prod_primes {s : Finset ℕ} {p : ℕ} (prime_p : p.Prime) :
  ...
  . left
    apply Or.resolve_left
    . exact Nat.Prime.eq_one_or_self_of_dvd h₀.1 _ h
    exact Nat.Prime.ne_one prime_p
  right
  apply ih
  . exact h₀.2
...
theorem aux {m n : ℕ} (h₀ : m ∣ n) (h₁ : 2 ≤ m) (h₂ : m < n) : n / m ∣ n ∧ n / m < n := by
  have : n / m ∣ n := Nat.div_dvd_of_dvd h₀
  use this
